...
/**
 * Interface that structures the list of detailed errors returned to the client.
 */
...
export const validateMiddleware = (schema: ObjectSchema) => {
    return (req: Request, _res: Response, next: NextFunction): void => {
        ...
        if (error) {
            ...
            const validationError = new BadRequestError("Errors in the validation of input data");

            // We attach the structured details to the error object so they can be used by error.middleware.ts
            (validationError as unknown as { details: ValidationErrorDetail[] }).details = errorDetails;
